...
import (
	...
	"github.com/fatal-fruit/neutrino/engine"
	engine_module "github.com/fatal-fruit/neutrino/x/engine"
...
func NewApp(
	...
	//engineKeeper := engine_module.NewEngineKeeper(appCodec, runtime.NewKVStoreService(keys[engine_module.StoreKey]))
	...
	app.mm = module.NewManager(
		...
		//engine_module.NewAppModule(engineKeeper),
	)
	...
	// The modular version has some issues with access, lifecycle & storage
	// ctx := sdk.NewContext(app.CommitMultiStore(), cmtproto.Header{Time: time.Now()}, false, log.NewNopLogger())
	// engineDriver := engine_module.NewEngineDriver(engineKeeper)

	engineDriver := engine.NewDriver("", "", "", "", common.Address{})
	app.SetPrepareProposal(engineDriver.PrepareProposal)
	app.SetProcessProposal(engineDriver.ProcessProposal)
	app.SetPreBlocker(engineDriver.PreBlocker)
	app.SetPrecommiter(engineDriver.Commit)
